...
public class SteamService {
    ...
    public String resolveSteamId(String input) {
        ...
        if (input.matches("^\\d{17}$")) {
            log.debug("Input is already SteamID64: {}", input);
            return input;
        }

        // otherwise we consider it a vanity url
        log.debug("Treating input as vanity URL: {}", input);
    ...
    public List<SteamGame> getTopGamesByPlaytime(String steamId) {
        log.info("Getting games for SteamID: {}", steamId);
        ...
        } catch (SteamPrivateProfileException | SteamApiException e) {
            log.warn("Business exception for SteamID {}: ", steamId);
            throw e; // throwing known custom exceptions
        } catch (Exception e) {
            log.warn("Error while fetching games for SteamID: {}", steamId);
            throw new SteamApiException("Error getting games list."); // throwing other possible exceptions
        }
    }

    public String formatTopGamesMessage(List<SteamGame> topGames, String originalInput, String resolvedSteamId) {
